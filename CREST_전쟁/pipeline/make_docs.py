...
**스캔이 붙었다.** 한동안 없었다. 여기까지 붙이면 `_site` 가 GitHub Pages
한도 1GB 를 넘었기 때문인데, 기존 문서철의 지면을 `열람기/shrink_pages.py` 로
줄여 자리를 만든 뒤 `fetch_scans.py` 로 받았다. 아래 코드가 `scans.json` 을
읽어 면 목록을 붙인다. **아직 다 받지 않았다.** 받은 것만 붙고 나머지는 빈
채로 지나가니, 받기가 끝나면 이것을 다시 돌려야 한다.
...
PHASE = (("1950-09-01", "A", "밀린다 · 개전에서 낙동강까지"),
         ...
         ("1951-01-01", "C", "다시 밀린다 · 중공군 개입에서 후퇴까지"),
         ("1951-07-10", "D", "전선이 굳는다 · 1·4 후퇴에서 회담 전까지"),
         ("9999",       "E", "회담하며 싸운다 · 개성·판문점"))
